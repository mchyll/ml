...
for i in range(0, W_grid.shape[0]):
    for j in range(0, W_grid.shape[1]):
        loss_grid[i, j] = LinearRegressionModel(W_grid[i, j], b_grid[i, j]).loss(x_train, y_train)
plot2.plot_surface(W_grid, b_grid, loss_grid, cmap=cm.coolwarm)

plot2_loss, = plot2.plot(model.W, model.b, model.loss(x_train, y_train), 'o', color='purple')
...
def update_figure(event=None):
    ...
    for i in range(0, x_train.shape[0]):
        plot1_loss[i].set_data([x_train[i, 0], x_train[i, 0]], [y_train[i, 0], model.f(x_train[i, 0])])

    plot1_info.set_text(
        '$W=[%.2f]$\n$b=[%.2f]$\n$loss = \\sum_i(f(\\hat x^{(i)}) - \\hat y^{(i)})^2 = %.2f$' % (model.W[0, 0], model.b[0, 0], model.loss(x_train, y_train)))

    plot2_loss.set_data(model.W, model.b)
